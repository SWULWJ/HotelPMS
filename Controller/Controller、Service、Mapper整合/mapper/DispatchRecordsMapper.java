...
public interface DispatchRecordsMapper {
	//查询所有属性（包括成员的属性）
	@Select("SELECT * FROM dispatch_records")
	@Results({
	...
	DispatchRecords queryById(int id);
	...
	@Select("SELECT d.id did, s.id sid, location, start_time, end_time, type, details, "
			...
			+"WHERE s.staff_number=#{staffNumber} AND staff_id=s.id")
	@Results({
	...
	List<DispatchRecords> queryByStaffNumber(int staffNumber);
}
